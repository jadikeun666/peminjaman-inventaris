...
<div class="col-md-7">
    ...
    <form method="POST" action="{{ route('peminjaman.store') }}">
        ...
        {{-- Pilih Barang --}}
        <div class="mb-3">
            <label class="form-label fw-semibold">Barang yang Dipinjam</label>
            <select name="barang[0][id]" class="form-select" id="selectBarang" required
        ...
        <div class="mb-3">
            ...
            <input type="number" name="barang[0][jumlah]" id="jumlah"
        ...
        <div class="mb-3">
            ...
            <input type="date" name="tanggal_peminjaman" id="tglPinjam"
                   class="form-control"
                   value="{{ date('Y-m-d') }}"
                   min="{{ date('Y-m-d') }}"
                   onchange="hitungEstimasi()" required>
        ...
        <div class="mb-3">
            ...
            <input type="date" name="tanggal_pengembalian" id="tglKembali"
                   class="form-control"
        ...
        {{-- Estimasi Biaya --}}
        <div class="alert alert-info py-2 mb-4" id="estimasiBox" style="display:none;">
            <div class="d-flex justify-content-between">
                <span>Estimasi biaya sewa:</span>
                <strong id="estimasiHarga">Rp 0</strong>
            </div>
            <small class="text-muted" id="estimasiDetail"></small>
        </div>
...
function hitungEstimasi() {
    ...
    const hari = Math.round(
        (new Date(tglKembali) - new Date(tglPinjam)) / (1000 * 60 * 60 * 24)
    );
    ...
    document.getElementById('estimasiHarga').textContent  = `Rp ${fmt}`;
    document.getElementById('estimasiDetail').textContent =
        `${jumlah} unit × Rp ${new Intl.NumberFormat('id-ID').format(harga)} × ${hari} hari`;
    document.getElementById('estimasiBox').style.display = 'block';
}

// auto-select barang dari URL param
